...
internal object XiaoAiHooks {
    ...
    private fun executeRun(
        ...
        try {
            ...
            logger.warnThrottled("xiaoai_runtime_failed") {
                "小爱运行时桥接异常: type=${exception.safeLogType()}"
            }
            if (activeRun.get() == run && !run.cancelled.get()) {
                run.renderer.fail()
            }
    ...
    private fun resolveImage(
        ...
        val path = try {
            ...
            logger.warnThrottled("xiaoai_image_lookup_failed") {
                "小爱图片查找异常: type=${exception.safeLogType()}"
            }
            null
        }
        if (path.isNullOrEmpty()) return XiaoAiImages.missingPath()
    ...
    private fun schedulePendingResultDrains(
        ...
        longArrayOf(1_000L, 5_000L, 15_000L).forEach { delay ->
            handler.postDelayed({
                try {
                    executor.execute {
                        drainPendingResults(logger, classLoader)
                    }
                } catch (_: RejectedExecutionException) {
    ...
    private fun drainPendingResults(
        ...
        client.drainCompletedRuns()
            ...
            .forEach { completed ->
                ...
                client.ackResult(completed.result.runId.ifBlank { completed.handoff.runId })
            }
    }
    ...
    private fun resolveFloatManager(
        ...
        val uiManager = getInstance.invoke(null, context)
    ...
    private fun optionalString(optional: Any?): String {
        ...
        if (HookSupport.invokeNoArgs(optional, "isPresent") != true) return ""
    ...
    private data class ActiveRun(
        ...
        val client: AtomicReference<AgentRuntimeClient?> = AtomicReference(null),
