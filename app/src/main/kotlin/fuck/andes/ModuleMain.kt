...
class ModuleMain : XposedModule() {
    ...
    override fun onPackageReady(param: PackageReadyParam) {
        when (param.packageName) {
            ...
            // 【新增】联想天禧AI（想帮帮）支持
            ModuleConfig.LENOVO_TIANXI_PACKAGE -> {
                if (isCurrentPackageProcess(ModuleConfig.LENOVO_TIANXI_PACKAGE)) {
                    logger.info("检测到天禧AI进程，准备安装Hook")
                    // 这里将添加天禧AI的Hook逻辑
                    // 目前先记录日志，后续需要实现具体的Hook逻辑
                }
            }
        }
    }
    ...
    private fun shouldKeepLifecycleCallbacks(param: ModuleLoadedParam): Boolean {
        ...
        return processName == ModuleConfig.SYSTEM_UI_PACKAGE ||
            ...
            // 【新增】添加天禧AI包名到生命周期回调
            isPackageProcess(processName, ModuleConfig.LENOVO_TIANXI_PACKAGE)
    }
